...
def startSet():
  ...
  def update():
    if checkvar == True:
      ...
      total_var.set(str(int((1-(discount[0])/100)*1.1 * (foodCost + drinkCost + dessertCost))))
      if setup != False:
        try:
  ...
  def addEverything():
    ...
    totalStuff += str(
      str("Total ") + str(" - ") +
      str(int(1.1 * (foodCost + drinkCost + dessertCost))) + str("\n"))
  ...
  def createFile():
    try:
      text_file = filedialog.asksaveasfile(
        mode='w',
        initialdir='C://Users//STD004//Desktop',
        title='Save As',
